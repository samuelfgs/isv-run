...
const RegistrationForm: React.FC<RegistrationFormProps> = ({ onSubmit, openTerms }) => {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      setSubmitError(error.message || 'Erro ao enviar inscrição. Por favor, tente novamente.');
      window.scrollTo({ top: document.querySelector('form')?.offsetTop || 0, behavior: 'smooth' });
  ...
  return (
    <form onSubmit={handleSubmit} className="bg-white p-8 sm:p-12 rounded-[2.5rem] shadow-[0_20px_50px_rgba(0,0,0,0.05)] border border-slate-100 space-y-8">
      <div className="space-y-6">
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
          ...
          <div className="space-y-1.5">
            ...
            />
            {cpfError && <p className="text-xs text-red-500 font-medium ml-1">{cpfError}</p>}
          </div>
        </div>
        ...
        <div className="space-y-1.5">
          ...
          />
          {dateError && <p className="text-xs text-red-500 font-medium ml-1">{dateError}</p>}
        </div>
      </div>
      ...
      <div className="space-y-6">
        ...
        <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
          <div
            onClick={() => setFormData({ ...formData, modalidade: Modalidade.RUN })}
          ...
          >
            <div className="flex justify-between items-start mb-4">
              <div className={`w-6 h-6 rounded-full border-2 flex items-center justify-center transition-colors ${formData.modalidade === Modalidade.RUN ? 'border-blue-600 bg-blue-600' : 'border-slate-300'}`}>
                {formData.modalidade === Modalidade.RUN && <div className="w-2 h-2 bg-white rounded-full" />}
              </div>
          ...
          <div
            onClick={() => setFormData({ ...formData, modalidade: Modalidade.WALK })}
            className={`group p-6 rounded-3xl border-2 cursor-pointer transition-all ${
          ...
          >
            <div className="flex justify-between items-start mb-4">
              <div className={`w-6 h-6 rounded-full border-2 flex items-center justify-center transition-colors ${formData.modalidade === Modalidade.WALK ? 'border-blue-600 bg-blue-600' : 'border-slate-300'}`}>
                {formData.modalidade === Modalidade.WALK && <div className="w-2 h-2 bg-white rounded-full" />}
              </div>
              <svg className={`w-8 h-8 ${formData.modalidade === Modalidade.WALK ? 'text-blue-600' : 'text-slate-300'}`} fill="none" stroke="currentColor" viewBox="0 0 24 24"><path strokeLinecap="round" strokeLinejoin="round" strokeWidth="1.5" d="M3.055 11H5a2 2 0 012 2v1a2 2 0 002 2 2 2 0 012 2v2.945M8 3.935V5.5A2.5 2.5 0 0010.5 8h.5a2 2 0 012 2 2 2 0 104 0 2 2 0 012-2h1.064M15 20.488V18a2 2 0 012-2h3.064M21 12a9 9 0 11-18 0 9 9 0 0118 0z"/></svg>
            </div>
            <p className={`font-black text-lg ${formData.modalidade === Modalidade.WALK ? 'text-blue-600' : 'text-slate-400'}`}>Caminhada 5km</p>
            <p className="text-xs text-slate-500 mt-1 font-medium">Ritmo livre</p>
          </div>
        </div>
      </div>
      ...
      <div className="pt-4 space-y-6">
        <div className="space-y-2">
          <div className={`flex items-start gap-4 p-5 rounded-[1.5rem] border ${
            termsError ? 'bg-red-50 border-red-200' : 'bg-slate-50 border-slate-100'
          }`}>
            ...
            <label htmlFor="aceitaTermos" className="text-xs font-medium text-slate-500 leading-normal select-none cursor-pointer">
              Declaro que li e concordo com o <button type="button" onClick={openTerms} className="text-blue-600 font-bold hover:underline">Regulamento Oficial</button> da prova, assumindo total responsabilidade pela minha saúde e participação.
            </label>
          </div>
          {termsError && <p className="text-xs text-red-500 font-medium ml-1">{termsError}</p>}
        </div>

        <div className="space-y-4">
          <div className="flex justify-between items-center px-2">
            <span className="text-sm font-bold text-slate-400 uppercase tracking-widest">Total a pagar</span>
            <span className="text-2xl font-black text-slate-900">{formatPrice(process.env.NEXT_PUBLIC_PRICE || '70')}</span>
          </div>
          {submitError && (
